...
class CheckoutController extends Controller
{
    ...
    public function checkoutOrder(Request $request)
    {
        if (Auth::check()) {
            ...
            $user_id = Auth::id();
            try {
               $subtotal = Cart::subtotal(); // Assuming $subtotal is '1,485.00'
$subtotal = str_replace(',', '', $subtotal); // Remove commas
$subtotal = (float) $subtotal; // Convert to float

// Now, $subtotal should be 1485.00, which is in the correct format for a decimal column

                $tax = tax_amount($subtotal, $request->billing_country);
                $shipping_charge = delivery_charge($request->billing_country);
                $this->grand_total = $subtotal +(float)$tax + (float)$shipping_charge;
...
$deliveryCharge = str_replace(',', '', delivery_charge($request->country));
$taxAmount = str_replace(',', '', tax_amount(Cart::subtotal(), $request->country));
$couponAmount = str_replace(',', '', Session::get('CouponAmount'));

$data['total_cost'] = floatval($subTotal) + floatval($deliveryCharge) + floatval($taxAmount) - floatval($couponAmount);
$totalCostInBaseCurrency = floatval(str_replace(',', '', Cart::subtotal()) + 
                                   floatval(str_replace(',', '', delivery_charge($request->country))) +
                                   floatval(str_replace(',', '', tax_amount(Cart::subtotal(), $request->country))) -
                                   floatval(str_replace(',', '', Session::get('CouponAmount'))));

$data['total_cost_curr'] = currencyConverter($totalCostInBaseCurrency);
